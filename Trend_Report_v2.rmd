...
```{r Lab Testing Metrics, code=xfun::read_utf8('src/new_06_labtesting_daily.R'), eval=eval_if_trend, results = "hide", message=FALSE, warning=FALSE, fig.height = 9, fig.width = 20}
```

```{r Lab Testing Table, message=FALSE, warning=FALSE, eval=eval_if_trend}
labtesting_table<-format_labtesting_table(input_table=weeks_combined)
labtesting_table
```
...
cat(
paste0("Highlights:
Over the past week, the 7 day moving average of ",key_Can_avg_tests_per_day," tests have been performed for COVID-19 across Canada, for a total of ",key_Can_weekly_tests," tests. The weekly percent positivity (7MA) is ",key_Can_weekly_perc_positive))
```
...
<!-- ``` -->
 ...
 ```{r lab testing figure with R, code=xfun::read_utf8('src/national_testing_v2.R'), fig.cap=(key_lab_figure_footnote), results='asis', message=FALSE, warning=FALSE, fig.height = 9, fig.width = 20}
 ```
...
source("src/summary_bullets_v2.R")
```
