...
for (i in list_pt){
        ...
        table_filter_case <- data.frame(
                ...
                value = c(number(df_filter$Cases_Daily[df_filter$Date==max(df_filter$Date)],accuracy=1,big.mark=","),
        ...
        table_filter_death <- data.frame(
                desc = c(paste0("Reported on ", format(max(df_filter$Date),"%B %d")), "7-day moving average (per day):", "Weekly percent change"),
                value = c(number(df_filter$Deaths_Daily[df_filter$Date==max(df_filter$Date)],accuracy=1,big.mark=","),
